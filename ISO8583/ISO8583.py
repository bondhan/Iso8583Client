...
"""
    ...
    def setTransationType(self, type):
        ...
        if len(type) > 4:
            type = type[0:3]
            raise ISOErrors.ValueToLarge('Error: value up to size! MTI limit size = 4')
    ...
    def setBit(self, bit, value):
        ...
        if bit < 1 or bit > 128:
            raise ISOErrors.BitInexistent("Bit number %s dosen't exist!" % bit)
    ...
    def __setBitTypeLL(self, bit, value):
        ...
        if len(value) > 99:
            # value = value[0:99]
            raise ISOErrors.ValueToLarge('Error: value up to size! Bit[%s] of type %s limit size = %s' % (
    ...
    def getRawIso(self):
        ...
        if self.MESSAGE_TYPE_INDICATION == '':
            raise ISOErrors.InvalidMTI('Check MTI! Do you set it?')
    ...
    def redefineBit(self, bit, smallStr, largeStr, bitType, size, valueType):
        ...
        if bit == 1 or bit == 64 or bit < 0 or bit > 128:
            raise ISOErrors.BitInexistent("Error %d cannot be changed because has a invalid number!" % bit)
        ...
        for cont in range(2, 129):
            if self.BITMAP_VALUES[cont] != self._BIT_DEFAULT_VALUE:
                ...
                if self.getBitType(cont) == 'LLL':
                    ...
                    if valueSize > self.getBitLimit(cont):
                        raise ISOErrors.ValueToLarge("This bit is larger than the especification!")
    ...
    def setIsoContent(self, iso):
        ...
        """
        if len(iso) < 20:
            raise ISOErrors.InvalidIso8583('This is not a valid iso!!')
        if self.DEBUG == True:
    ...
    def getBit(self, bit):
        ...
        if isThere:
            ...
            raise ISOErrors.BitNotSet("Bit number %s was not set!" % bit)

    ################################################################################################

    def pad_zero(self, s, length):
        return str(s).zfill(length)

    ################################################################################################
    # Method that return ISO8583 to TCPIP network form, with the size in the beginning.
    def getNetworkISO(self, variant='xlink'):
        ...
        length_prefix =str(length).zfill(4).encode()
        res = length_prefix + asciiIso.encode()

        # if variant == 'xlink':
        #     length_prefix =str(length).zfill(4).encode()
        #     res = length_prefix + asciiIso.encode()
        # else:
        #     hex_str = f"{length:04X}"  # '0020'
        #     byte_value = hex_str.encode()  # b'0020'
        #     res = byte_value + asciiIso.encode()

            # padded_length = self.pad_zero(length, 4)
            # formatted_data = padded_length + asciiIso.upper()
            # msg = formatted_data.encode()
            # res = len(msg) + msg
    ...
    def setNetworkISO(self, iso, variant='xlink'):
        ...
        if len(iso) < 24:
            raise ISOErrors.InvalidIso8583('This is not a valid iso!!Invalid Size')

        # if variant == 'artajasa':
        #     sz = 2
        #     size = int(iso[0:sz].decode('utf-8'), 16)
        # else:
        sz = 4
        size = int(iso[0:sz])

        self.setIsoContent(iso[sz:])
